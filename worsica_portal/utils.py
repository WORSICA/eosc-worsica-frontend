...
def is_bad_input(bad_input):
    ...
    FILTER_SQL_TERMS = ['SELECT ', 'FROM ', 'WHERE ', 'DELETE ', 'IN ',
                        'AND ', 'OR ', 'ALTER ', 'CREATE ', 'DROP ', 'SLEEP ', 'DELAY ']
...
def notify_set_active(new_user, active):
    ...
    if active:
        ...
        plain_content = ('Congratulations, your account ' +
                         new_user.username+' is activated. You can login.\n')
        notify_users([new_user.email], subject, plain_content)
    else:
        subject = 'Account deactivated'
        plain_content = ('Sorry, your account '+new_user.username +
                         ' is deactivated. For more information, enter in contact with the support (worsica@lnec.pt).\n')
        notify_users([new_user.email], subject, plain_content)
