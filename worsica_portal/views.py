	...
	try:
		...
		for sf in subfolders:
			...
			for ssf in subsubfolders:
				...
				if (r.status_code == 201 or r.status_code == 405):
					print('[_create_user_repository_frontend]: folder '+pathsf2+' does not exist, create')
...
def login(request):
	...
	if check_is_user_authenticated(request.user):
		if check_has_user_filled_required_information(request.user):		
			...
			c = _create_message(c, "error", 'You havent finished your registration. If you are an EGI user, please login as EGI to finish.')
		...
		#if (request.session.get('msgtype')):	
		#	c['msgtype'] = request.session.get('msgtype')
		#	request.session.pop('msgtype') #time to cleanup request.session to avoid loading the message
		#if (request.session.get('message')):
		#	c['message'] = request.session.get('message')
		#	request.session.pop('message') #time to cleanup request.session to avoid loading the message
	#c.update(csrf(request))
...
def auth_view(request):
	if '@' in request.POST.get('email',''):
		try:
			...
			#print(find_user)
			user = auth.authenticate(username=find_user.username, password=request.POST.get('password',''))
			#print(user)
			if user is not None:
				auth.login(request, user)
				if check_has_user_activated(user): #if user.is_active: #if request.user.is_active:
					if check_has_user_confirmed_registration(user): #uprofile = worsica_portal_models.UserProfile.objects.get(user = request.user) #if uprofile.confirm_registration:
						if not check_has_user_read_disclaimer(user): #if not uprofile.read_disclaimer:
							return redirect('/portal/disclaimer')
						else:
							if (request.GET.get('next')): #check if theres a redirect
								return redirect(request.GET.get('next'))
						...
						request.session = _create_message(request.session, "error", 'Error: User has not confirmed yet the registration!')
						return redirect('/accounts/login/')
				else:
					#auth.logout(request)
					request.session = _create_message(request.session, "error", 'Error: User not yet activated!')
					return redirect('/accounts/login/')
			else:
				request.session = _create_message(request.session, "error", 'Error: Invalid email/password! Or if you already have an account, make sure you confirmed it and/or was approved by the administration.')
				return redirect('/accounts/login/')
		except Exception as e:
			request.session = _create_message(request.session, "error", 'Error: '+str(e))
			return redirect('/accounts/login/')
	else:
		request.session = _create_message(request.session, "error", 'Error: Please provide an email address!')
		return redirect('/accounts/login/')
...
def register_post(request):
	if request.user.is_authenticated:
		...
		#name 	#surname 	#email	#affiliatio	#password	#cpassword
		user = None
		user_created = False
		...
		try:
			...
			if is_form_inputs_good:
				...
				get_user_by_username = User.objects.filter(username=utils.sanitize_input_email(formdata['email']))
				get_user_by_email = User.objects.filter(email=utils.sanitize_input_email(formdata['email']))
				...
				if len(get_user_by_username)>0:
					...
					user, user_created = User.objects.get_or_create(username=utils.sanitize_input_email(formdata['email']), email = utils.sanitize_input_email(formdata['email']))
					if user_created:
						...
						if up_created:
							...
							if not userprofile.confirm_registration: #user.is_active:
								...
								try:
									...
									jsonmsg = _create_message(jsonmsg, 'notice','Success! Check SPAM, in a few minutes, you will receive an email with a link to confirm account.')
									return HttpResponse(json.dumps(jsonmsg), content_type='application/json')
								except Exception as e:
									print(e)
		...
		if check_is_user_authenticated(request.user):
			...
			if user.is_active:
				...
				if userprofile.confirm_registration:
					request.session = _create_message(request.session, "error", 'Error: User has confirmed registration!' )
					return redirect('/accounts/login/')
				else:
					userprofile.confirm_registration = True
					userprofile.save()
					#send email to set new password
					activation_link = request.build_absolute_uri("/accounts/activate/"+str(uidb64)+'/'+str(token))
					utils.notify_confirm_registration(user, userprofile, activation_link)
					request.session = _create_message(request.session, "notice", 'Success, you confirmed your account! Now you need to wait for administration to approve your account. You will receive soon an email confirming that and to login!' )
					return redirect('/accounts/login/')
	except Exception as e:
		request.session = _create_message(request.session, "error", 'Error: User with that id does not exist')
...
def recovery_post(request):
	if check_is_user_authenticated(request.user):
		...
		try:
			...
			token, uidb64 = user_token_encrypt(user)
			password_recover_link = request.build_absolute_uri("/accounts/recovery_set_pwd/"+str(uidb64)+'/'+str(token))
			utils.notify_user_password_recover(user, password_recover_link)
			#send email to set new password
			jsonmsg = _create_message(jsonmsg, 'notice', 'Success! An email for password recovery was sent, please check on your mail box. Check SPAM.')
			return HttpResponse(json.dumps(jsonmsg), content_type='application/json')
			#return redirect('/accounts/login/')
		except Exception as e:
...
def recovery_set_pwd(request, uidb64, token):
	try:
		...
		request.session = _create_message(request.session, "error", 'Error: User with that id does not exist/Invalid token' )
		return redirect('/accounts/login/')
def recovery_set_pwd_post(request, uidb64, token):
	...
	try:
		if check_is_user_authenticated(request.user):
			...
			#send email 			
			#return redirect('/accounts/login/')
			jsonmsg = _create_message(jsonmsg, 'notice', 'Success! You changed the password, please login.')
			return HttpResponse(json.dumps(jsonmsg), content_type='application/json')
	except Exception as e:
		jsonmsg = _create_message(jsonmsg, "error", 'Error: '+str(e))
		return HttpResponse(json.dumps(jsonmsg), content_type='application/json')
...
def login_egi(request):
	request.session = _create_message(request.session, "error", 'An error occured during authentication. Try again.')
	try:
		#print(request.session.keys())
		#use access_token for dev (keycloak) or openid for production (mitreid)
		KEY_TOKEN = 'access_token' #if settings.DEBUG else 'openid_token'
		if KEY_TOKEN in request.session:
			...
			#BUG: auth-oidc creates a new user account with is_active enabled?
			if check_has_user_filled_required_information(request.user):
				#if userprofile.confirm_registration:
				if check_has_user_confirmed_registration(request.user):
					...
					if check_has_user_activated(request.user):
						...
						request.session = _create_message(request.session, "error", 'Your account is not activated.')
						return redirect('/accounts/login/')
				else:
					request.session = _create_message(request.session, "error", 'You need to confirm your registration.')
					return redirect('/accounts/login/')
				...
				get_user_info = requests.get(settings.AUTH_SERVER_USERINFO, headers=headers, allow_redirects=False)
				#print('---------------')
				#print('headers')
				print(headers)
				#print('---------------')
				#print('url')
				print(settings.AUTH_SERVER_USERINFO)
				...
				if get_user_info.status_code == 200:
					...
					user_info_sub = user_info['voperson_id'] #if settings.DEBUG else user_info['sub']
					...
					new_user.save()
					# Send fillout form
					request.session = _create_message(request.session, 'notice', 'Fill the required data and submit.')
					# If UserProfile already exists but no affiliation info exists: send out fill form again
					try:
						new_userprofile = worsica_portal_models.UserProfile.objects.get(user=new_user)
						if new_userprofile.affiliation and new_userprofile.affiliation_country:
							...
							request.session = _create_message(request.session, 'error', 'Your registration is still pending!')
							return redirect('/accounts/login/') 
					except Exception:
						print('skip...')
					c = _purge_message(request)
					...
					#auth.logout(request)
					request.session = _create_message(request.session, "error", 'Error: userinfo request status not 200')
		...
		return redirect('/accounts/login/') #render(request, 'front/login_egi.html', {'response': msg})
	

# Complete EGI Registration
def login_egi_complete(request):
# Try to update info if already exists
	#request.session = _create_message(request.session, "error", 'An error occured, try again later!')
	#print('user_info_sub')
	#print(request.session.get('user_info_sub'))
	if request.POST:
		request.encoding='utf-8'
		formdata = request.POST
		...
		if is_form_inputs_good:
			...
			jsonmsg = _create_message(jsonmsg, 'error', 'Error! Invalid inputs on form found, aborting EGI registration!.')
			return HttpResponse(json.dumps(jsonmsg), content_type='application/json')
...
def get_xls_metrics(monthYear):	
	...
	endDate = monthYear[0]+'-'+monthYear[1]+'-'+str(calendar.monthrange(int(monthYear[0]), int(monthYear[1]))[1])
	metrics = get_metrics(monthYear)

	response = HttpResponse(content_type='application/ms-excel')
	response['Content-Disposition'] = 'attachment; filename="metrics-'+monthYear[0]+'-'+monthYear[1]+'.xls"'
	...
	font_style = xlwt.XFStyle()
	rows = [
		['MU_NUS','Number of different direct users who have accessed the service in a given period.',metrics['mu_nus'],'Users/month'],
		['MU_NUSA','Accumulative Number of direct different users who have accessed the service since PM6.',metrics['mu_nusa'],'Users'],
		['MU_NCEA','Accumulative Number of different centers where the users are based since PM6.',metrics['mu_ncea'],'Centers'],
		['MU_NCOA','Accumulative Number of different countries of origin of the users since PM6.',metrics['mu_ncoa'],'Countries'],
		['MU_NRUSA','Accumulative number of different users that accessed the service more than once since PM6.',metrics['mu_nrusa'],'Users'],
	]
	for row in rows:
		row_num += 1
...
def get_metrics(monthYear):
	...
	q = request_models.Request.objects.filter(time__lte = endDate, path__icontains = '/portal').exclude(user_id=None)
	...
	mu_ncoa = q.order_by('user__userprofile__affiliation_country').distinct('user__userprofile__affiliation_country')
	#print(mu_ncoa)
...
def upload_roi_coords(request):
	if request.method == 'POST':		
		try:
			...
			print(tmpfile)
			#shp_area_name = slugify(myfile.name.split('.')[0])
			#print(shp_area_name)
			MAX_FILESIZE = 512
...
def generate_topography_roi_simulation(request):
	try:
		...
		interm_generate_topography = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/generate_topography'
...
def get_user_rois(request):
	...
	aoss = worsica_portal_models.AreaOfStudy.objects.filter(**kwargs).filter(Q(user__id=request.user.id))	
	aoss2 = [a for a in aoss]
	#shared with
	shared_sims = worsica_portal_models.Simulation.objects.filter(Q(shared_with__id__in=[request.user.id])&Q(aos__service=service)&Q(is_visible=True))
...
@login_required
def get_satellite_imagery(request):
	#TODO, avoid using the geosgeometry
	bD,eD = request.GET.get('beginDate'), request.GET.get('endDate')
	uX,uY = request.GET.get('uX'), request.GET.get('uY')
	lX,lY = request.GET.get('lX'), request.GET.get('lY')
	workspace_id = request.GET.get('workspace_id')
	#imagery = worsica_portal_models.SatelliteImageSet.objects.filter(sensingDate__range=(bD,eD), workspace_id=workspace_id)
	imageryJson = []
	#userRoi = GEOSGeometry('{ "type": "Polygon", "coordinates": [[['+lX+','+lY+'],['+lX+','+uY+'],['+uX+','+uY+'],['+uX+','+lY+'],['+lX+','+lY+']]]}')
	#worsica_logger.info('[get_satellite_imagery]: '+userRoi)
	#for img in imagery:
	#	worsica_logger.info('[get_satellite_imagery]: ---------')
	#	worsica_logger.info('[get_satellite_imagery]: '+img.name)
		#imgRoi = GEOSGeometry('{ "type": "Polygon", "coordinates": [[['+str(img.lowerXcoordinate)+','+str(img.lowerYcoordinate)+'],['+str(img.lowerXcoordinate)+','+str(img.upperYcoordinate)+'],['+str(img.upperXcoordinate)+','+str(img.upperYcoordinate)+'],['+str(img.upperXcoordinate)+','+str(img.lowerYcoordinate)+'],['+str(img.lowerXcoordinate)+','+str(img.lowerYcoordinate)+']]]}')
		#worsica_logger.info('[get_satellite_imagery]: '+imgRoi)
		#if (imgRoi.intersects(userRoi)):
		#	imageryJson.append({'name': img.reference, 'custom_name': img.name, 'uuid': img.uuid, 'date': str(img.sensingDate) })#do not use 'name' here, it will be used to show to user
...
def proxy_get_imageset_thumbnail(request, uuid):
	...
	quicklook_url = "https://scihub.copernicus.eu/dhus/odata/v1/Products('"+uuid+"')/Products('Quicklook')/$value"
...
def _list_esa_query_products(footprint, date, platformname, s1_producttype=None, s1_sensoroperationalmode = None, s2_processinglevel=None, s2_cloudcoverpercentage=None):
	...
	for user in esaCredentials.keys():
		try:
			...
			api = SentinelAPI(esaCredentials[user]["user"], esaCredentials[user]["password"])#, 'https://scihub.copernicus.eu/dhus')
			#sentinel2
			if platformname == 'Sentinel-2':
				p = api.query(footprint, date=date, platformname=platformname, processinglevel=s2_processinglevel, cloudcoverpercentage=s2_cloudcoverpercentage)
				querySuccess = True
				break
			#sentinel1
			elif platformname == 'Sentinel-1':
				p = api.query(footprint, date=date, platformname=platformname, producttype=s1_producttype, sensoroperationalmode=s1_sensoroperationalmode)
				querySuccess = True
				break
		except Exception as e:
			print(traceback.format_exc())
	...
	if querySuccess:
		...
		#worsica_logger.info('[_list_esa_query_products]: query fail!')
		#continue
		worsica_logger.info('[_list_esa_query_products]: tried all available accounts with no sucess, giving up!')
		raise Exception('Query failed')
			
def _start_esa_imagery_search(bD, eD, uX, uY, lX, lY, minCloudCov, maxCloudCov, platformname, level=None, sensorOM=None):
	footprint = geojson_to_wkt({ "type": "Polygon", "coordinates": [ [ [lX, lY], [lX, uY], [uX, uY], [uX, lY], [lX, lY] ] ] })
	imageryJson = []
	if platformname == 'Sentinel-2':
		#level = request.GET.get('processinglevel') #'Level-1C,Level-2A'
		ls = level.split(',') #['Level-1C','Level-2A']
		#if chosen l1c and l2a, give priority to l2a
		...
		for l in ls:
			print(l)
			#p = api.query(footprint, date=(bD, eD), platformname=platformname, processinglevel=l, cloudcoverpercentage=(minCloudCov, maxCloudCov))#, order_by='-beginposition')
			#products = OrderedDict(sorted(p.items(), key=lambda v: v[1]['beginposition'], reverse=True))
			products = _list_esa_query_products(footprint, date=(bD, eD), platformname=platformname, s2_processinglevel=l, s2_cloudcoverpercentage=(minCloudCov, maxCloudCov))
			loadedProducts = {}
			for Id in products.keys():
				splitIdentifier = products[Id]["identifier"].split('_')#e.g: S2A_MSIL2A_20180301T112111_N0206_R037_T29SPC_20180301T114356
				...
				#print(str(products[Id]["processinglevel"]))
				#print(splitIdentifier[2]+"_"+splitIdentifier[5])
				if (splitIdentifier[2]+"_"+splitIdentifier[5] not in loadedProducts): #if not repeated, load it 
					worsica_logger.info('[get_satellite_imagery_from_esa]: add '+splitIdentifier[2]+"_"+splitIdentifier[5]+' (pd: '+splitIdentifier[6]+')')
					loadedProducts[splitIdentifier[2]+"_"+splitIdentifier[5]] = jayson
					eD = products[Id]["beginposition"].strftime("%Y%m%d") #get the least recent for the next query l1c
				else:
		...
		#level = request.GET.get('producttype') #'GRD,'
		#sensorOM = request.GET.get('sensoroperationalmode') #'IW,'
		ls = level.split(',') #['Level-1C','Level-2A']
		sms = sensorOM.split(',')
		for l in ls:
			for sm in sms:
				#p = api.query(footprint, date=(bD, eD), platformname=platformname, producttype=l, sensoroperationalmode=sm)
				#products = OrderedDict(sorted(p.items(), key=lambda v: v[1]['beginposition'], reverse=True))
				products = _list_esa_query_products(footprint, date=(bD, eD), platformname=platformname, s1_producttype=l, s1_sensoroperationalmode=sm)
				loadedProducts = {}
				for Id in products.keys():
					splitIdentifier = products[Id]["identifier"].split('_')#e.g: S2A_MSIL2A_20180301T112111_N0206_R037_T29SPC_20180301T114356
					_extent, _startAt = utils.parse_wkt_to_extent(str(products[Id]["footprint"]))
...
def get_satellite_imagery_from_esa(request):
	bD,eD = request.GET.get('beginDate').replace("-",""), request.GET.get('endDate').replace("-","")
	...
	level = (request.GET.get('processinglevel') if platformname == 'Sentinel-2' else request.GET.get('producttype')) #'Level-1C,Level-2A'
	#level = request.GET.get('producttype') #'GRD,'
	sensorOM = request.GET.get('sensoroperationalmode') #'IW,'
	imageryJson = _start_esa_imagery_search(bD, eD, uX, uY, lX, lY, minCloudCov, maxCloudCov, platformname, level, sensorOM)
	#esaCredentials = SSecrets.getCredentials()['user1']
	#api = SentinelAPI(esaCredentials["user"], esaCredentials["password"], 'https://scihub.copernicus.eu/dhus')
...
def _build_job_submission(jsonReq, simulation, roi):
	...
	#json['roi'] = 'POLYGON (('+extentF[0]+' '+extentF[1]+','+extentS[0]+' '+extentF[1]+','+extentS[0]+' '+extentS[1]+','+extentF[0]+' '+extentS[1]+','+extentF[0]+' '+extentF[1]+'))'
	#upperXcoordinate = extentS[0] #upperYcoordinate = extentS[1]
	#lowerXcoordinate = extentF[0] #lowerYcoordinate = extentF[1]
...
def edit_simulation(request, roi_id, simulation_id):
	try:
		...
		if jsonReq_roi_id != roi_id:
			...
			reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/edit', json=payload)
			cs = reqPOST.json()
			if cs['alert'] == 'created':
				s = cs['job_submission']
				createdSim.append(s)
...
def create_simulation(request, roi_id):
	...
	try:
		...
		if jsonReq_roi_id != roi_id:
			...
			reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/create', json=payload)
			cs = reqPOST.json()
			#print(cs)
			if cs['alert'] == 'created':
				s = cs['job_submission']
				#if success, update simulation_id
				simulation.name = s['name']#'simulation'+str(satelliteImageSet.id)+'-'+satelliteImageSet.name
...
def get_simulation_dataverse_datasets(request, roi_id, simulation_id):
	try:
		...
		if not is_owned_by_user:
			...
			reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/dataverses')
...
def add_simulation_dataverse_datasets(request, roi_id, simulation_id):
	try:
		...
		if not is_owned_by_user:
			...
			payload['owner'] = {'name': user.first_name+' '+user.last_name, 'email': user.email, 'affiliation': up.affiliation}
			reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/dataverses/submit', json=payload)
			cs = reqPOST.json()
			createdDataverse = []
			print(cs)
			if cs['state'] == 'submitted':
				#createdDataverse = cs['response']
				#if success, update simulation_id
				#createdDataverse.append(cs['response'])
				print('submitted')
			else:
				failedCreation = True
...
def remove_user_share_simulation(request, roi_id, simulation_id, existing_user_id):
	try:
		...
		roi = get_user_roi(rois, roi_id) #rois.get(pk = roi_id)
		#check if you are not trying to cheat the system
		#new_share_user_id = int(jsonReq['new_share_user_id'])
		is_owned_by_user = (roi.user.id == request.user.id)
		if not is_owned_by_user:
		...
		#elif request.user.id == new_share_user_id:
		#	return HttpResponse(json.dumps({"alert":"error", "exception": "You cannot share your own simulation to yourself!"}), content_type='application/json')
		else:
			...
			if existing_user is None:		
				...
				#, "shared_with_users":[{'id': u.id, 'name': u.first_name, 'email': u.email} for u in simulation.shared_with.all()]
	except Exception as e:
		print(traceback.format_exc())
		return HttpResponse(json.dumps({"alert":"error", "exception": "worsica-web:"+str(e)}), content_type='application/json')
...
def waterleak_edit_leak_detection(request, roi_id, simulation_id, leak_detection_id):
	try:
		...
		if jsonReq_roi_id != roi_id:
			...
			roi = get_user_roi(rois, roi_id) #rois.get(pk = roi_id)			
			simulation = worsica_portal_models.Simulation.objects.get(aos = roi, pk = simulation_id)
			leakdetection = worsica_portal_models.LeakDetection.objects.get(simulation = simulation, id = leak_detection_id)
			...
			reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/edit', json=payload)
			cs = reqPOST.json()
			print(cs)
			if cs['alert'] == 'created':
				s = cs['leak_detection']
	...
	try:
		...
		if jsonReq_roi_id != roi_id:
			...
			simulation = worsica_portal_models.Simulation.objects.get(
				aos = roi,
				id = simulation_id #jsonReq['step1ImageSelection']['simulation_id']
			)
			...
			leakdetection.reference = roi.service+'-aos'+str(roi.id)+'-simulation'+str(simulation.id)+'-leakdetection'+str(leakdetection.id)
			leakdetection.save()
			...
			reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/create', json=payload)
			cs = reqPOST.json()
			print(cs)
			if cs['alert'] == 'created':
				s = cs['leak_detection']
				#if success, update simulation_id
				leakdetection.name = s['name']#'simulation'+str(satelliteImageSet.id)+'-'+satelliteImageSet.name
...
def waterleak_check_existing_simulation(request, roi_id):
	...
	try:
		...
		simulations = worsica_portal_models.Simulation.objects.filter(aos__id=roi_id, aos__service = 'waterleak', is_visible=True)
...
def get_simulations(request, roi_id):
	try:
		...
		_sims = worsica_portal_models.Simulation.objects.filter(is_visible=True)
		if is_owned_by_user:
			simulations = worsica_portal_models.Simulation.objects.filter(aos__id=roi_id)
		else:
			simulations = worsica_portal_models.Simulation.objects.filter(shared_with__id__in=[user_id], aos__service=service_type, is_visible=True)
		#simulations = [s for s in own_simulations]#+[s2 for s2 in shared_simulations]
		
		roi_polygon = str(aos.upperYcoordinate)+'|'+str(aos.upperXcoordinate)+'|'+str(aos.lowerYcoordinate)+'|'+str(aos.lowerXcoordinate)
		list_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/list?service_type='+service_type+'&roi_polygon='+roi_polygon
		...
		for simulation in simulations:
			if simulation.job_submission_id is not None:
				reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id), params=request.GET)
				j = reqGET.json()				
				if (j['alert'] != 'error'):
					...
					if 'leakDetections' in j:
						for k in j['leakDetections']:#reverse ld_id to provide the correct id on portal side
							try:
								pld = worsica_portal_models.LeakDetection.objects.get(interm_leak_detection_id=k['ld_id'])						
								k['portal_ld_id']=(pld.id if pld is not None else None)
							except Exception as e:
								k['portal_ld_id']=None
								pass
							for l in k['outputs']:
								l['is_owned_by_user'] = is_owned_by_user
...
def show_simulation(request, roi_id, simulation_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		is_owned_by_user = (simulation.aos.user.id == request.user.id) or (request.user in simulation.shared_with.all())
		if not is_owned_by_user:
...
def show_simulation_details(request, roi_id, simulation_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/details')
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def show_simulation_procimageset_coastline(request, roi_id, simulation_id, intermediate_process_imageset_id, shp_mpis_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/coastline/'+shp_mpis_id, params=request.GET)
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def clone_cleanup_simulation_coastlines(request, roi_id, simulation_id):
	try:
		...
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/clone_cleanup_all', json=payload)
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def clone_cleanup_simulation_procimageset_coastline(request, roi_id, simulation_id, intermediate_process_imageset_id, shp_mpis_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		payload = json.loads(request.body.decode('utf-8'))
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/coastline/'+shp_mpis_id+'/clone_cleanup', json=payload)
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def delete_cleanup_simulation_procimageset_coastline(request, roi_id, simulation_id, intermediate_process_imageset_id, shp_mpis_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/coastline/'+shp_mpis_id+'/delete_cleanup')
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def download_cleanup_simulation_procimageset_coastline(request, roi_id, simulation_id, intermediate_process_imageset_id, shp_mpis_id):
	try:
		...
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		interm_download_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/coastline/'+str(shp_mpis_id)+'/download_cleanup'
		response = requests.get(interm_download_url, params=request.GET, stream=True)
		if response.status_code == 200:
			show_download = response.json()
			r = requests.get(show_download['url'], stream=True, auth=(show_download['user'], show_download['pwd']))
			appropriateFileName = str(simulation.aos.name)+'-'+str(simulation.name)+'-'+show_download['appropriateFileName']
...
def show_simulation_procimageset_raster_leakdetection(request, roi_id, simulation_id, leak_detection_id, intermediate_process_imageset_id, determine_leak, leak_type, intermediate_raster_type, intermediate_raster_id):
	try:
		...
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(ld.interm_leak_detection_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+determine_leak+'/'+leak_type+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id))
		show_simulation = reqGET.json()
		show_simulation['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+show_simulation['response']['url'])#raster/tiles/'+str(intermediate_raster_id))
...
def show_simulation_procimageset_raster_leakdetection_leakpoints(request, roi_id, simulation_id, leak_detection_id, intermediate_process_imageset_id, determine_leak, leak_type, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(ld.interm_leak_detection_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+determine_leak+'/'+leak_type+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id)+'/leak_points/show', params=request.GET)
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'state': 'error','roi_id': roi_id, 'id': simulation_id, 'error':str(e)}), content_type='application/json')
...
def download_simulation_procimageset_raster_leakdetection_leakpoints(request, roi_id, simulation_id, leak_detection_id, intermediate_process_imageset_id, determine_leak, leak_type, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(ld.interm_leak_detection_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+determine_leak+'/'+leak_type+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id)+'/leak_points/download', params=request.GET)
		show_request = reqGET.json()
		response = HttpResponse(show_request['kml'], content_type='application/vnd.google-earth.kml+xml')
		response['Content-Disposition'] = 'attachment; filename="{0}"'.format(simulation.aos.name+'-'+show_request['filename'])
		return response
	except Exception as e:
		return HttpResponse(json.dumps({'state': 'error','roi_id': roi_id, 'id': simulation_id, 'error':str(e)}), content_type='application/json')
...
def change_simulation_procimageset_raster_leakdetection_leakpoint(request, roi_id, simulation_id, leak_detection_id, intermediate_process_imageset_id, determine_leak, leak_type, intermediate_raster_type, intermediate_raster_id, leak_point_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(ld.interm_leak_detection_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+determine_leak+'/'+leak_type+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id)+'/leak_points/'+str(leak_point_id)+'/change_visibility', params=request.GET)
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'state': 'error','roi_id': roi_id, 'id': simulation_id, 'error':str(e)}), content_type='application/json')
...
def show_simulation_ucprocimageset_raster_leakdetection(request, roi_id, simulation_id, leak_detection_id, intermediate_ucprocess_imageset_id, determine_leak, leak_type, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(ld.interm_leak_detection_id)+'/ucprocessimageset/'+str(intermediate_ucprocess_imageset_id)+'/'+determine_leak+'/'+leak_type+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id))
		show_simulation = reqGET.json()
		show_simulation['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+show_simulation['response']['url'])#raster/tiles/'+str(intermediate_raster_id))
		show_simulation['simulation_id'] = simulation_id
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def show_simulation_ucprocimageset_raster_leakdetection_leakpoints(request, roi_id, simulation_id, leak_detection_id, intermediate_ucprocess_imageset_id, determine_leak, leak_type, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(ld.interm_leak_detection_id)+'/ucprocessimageset/'+str(intermediate_ucprocess_imageset_id)+'/'+determine_leak+'/'+leak_type+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id)+'/leak_points/show', params=request.GET)
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'state': 'error','roi_id': roi_id, 'id': simulation_id, 'error':str(e)}), content_type='application/json')
...
def show_simulation_procimageset_raster(request, roi_id, simulation_id, intermediate_process_imageset_id, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id))
		show_simulation = reqGET.json()
		show_simulation['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+show_simulation['response']['url'])#raster/tiles/'+str(intermediate_raster_id))
		show_simulation['simulation_id'] = simulation_id
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def show_simulation_procimageset_raster_histogram(request, roi_id, simulation_id, intermediate_process_imageset_id, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id)+'/show_histogram')
		show_simulation = reqGET.json()
		#show_simulation['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+show_simulation['response']['url'])#raster/tiles/'+str(intermediate_raster_id))
		show_simulation['simulation_id'] = simulation_id
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def create_threshold_simulation_procimageset(request, roi_id, simulation_id, intermediate_process_imageset_id, intermediate_raster_type):
	try:
		jsonReq = json.loads(request.body.decode('utf-8'))
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+str(intermediate_raster_type)+'/create_threshold', json=jsonReq)
		show_simulation = reqGET.json()
		show_simulation['simulation_id'] = simulation_id
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def download_threshold_simulation_procimageset(request, roi_id, simulation_id, intermediate_process_imageset_id, intermediate_raster_type):
	try:
		CHUNK_SIZE = 5*1024*1024 #5MB
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		interm_download_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/'+str(intermediate_raster_type)+'/download_threshold'
		response = requests.get(interm_download_url, params=request.GET, stream=True)
		if response.status_code == 200:
			show_download = response.json()
			r = requests.get(show_download['url'], stream=True, auth=(show_download['user'], show_download['pwd']))
...
def show_simulation_procimageset_raster_climatology(request, roi_id, simulation_id, intermediate_process_imageset_id, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/averageprocessimageset/'+str(intermediate_process_imageset_id)+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id))
		show_simulation = reqGET.json()
		show_simulation['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+show_simulation['response']['url'])#raster/tiles/'+str(intermediate_raster_id))
		show_simulation['simulation_id'] = simulation_id
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def show_simulation_topography_raster(request, roi_id, simulation_id, intermediate_gt_id, intermediate_raster_type, intermediate_raster_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/generatetopography/'+str(intermediate_gt_id)+'/'+str(intermediate_raster_type)+'/'+str(intermediate_raster_id))
		show_simulation = reqGET.json()
		show_simulation['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+show_simulation['response']['url'])#raster/tiles/'+str(intermediate_raster_id))
		show_simulation['simulation_id'] = simulation_id
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
...
def proxy_get_intermediate_download_topography(request, roi_id, simulation_id, intermediate_gt_id):
	try:
		CHUNK_SIZE = 5*1024*1024 #5MB
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		interm_download_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/generatetopography/'+str(intermediate_gt_id)+'/download_topography'
		response = requests.get(interm_download_url, params=request.GET, stream=True)
		if response.status_code == 200:
			show_download = response.json()
			r = requests.get(show_download['url'], stream=True, auth=(show_download['user'], show_download['pwd']))
...
def proxy_get_intermediate_delete_topography(request, roi_id, simulation_id, intermediate_gt_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/generatetopography/'+str(intermediate_gt_id)+'/delete_topography')
		show_simulation = reqGET.json()
		return HttpResponse(json.dumps(show_simulation), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'state': 'error','roi_id': roi_id, 'id': simulation_id, 'error':str(e)}), content_type='application/json')
...
def proxy_get_intermediate_procimageset_raster(request, intermediate_path):
	try:
		procimageset_raster_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/'+str(intermediate_path)
		#print(procimageset_raster_url)
...
def proxy_get_intermediate_download_simulation_products_climatology(request, roi_id, simulation_id,  intermediate_process_imageset_id):
	try:
		CHUNK_SIZE = 5*1024*1024 #5MB
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		interm_download_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/averageprocessimageset/'+str(intermediate_process_imageset_id)+'/download_products'
		response = requests.get(interm_download_url, params=request.GET, stream=True)
		if response.status_code == 200:
			show_download = response.json()
			r = requests.get(show_download['url'], stream=True, auth=(show_download['user'], show_download['pwd']))
...
def proxy_get_intermediate_download_simulation_products(request, roi_id, simulation_id,  intermediate_process_imageset_id):
	try:
		CHUNK_SIZE = 5*1024*1024 #5MB
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		interm_download_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/processimageset/'+str(intermediate_process_imageset_id)+'/download_products'
		response = requests.get(interm_download_url, params=request.GET, stream=True)
		if response.status_code == 200:
			show_download = response.json()
			r = requests.get(show_download['url'], stream=True, auth=(show_download['user'], show_download['pwd']))
...
def waterleak_run_leak_detection(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/run')
		run_ld = reqGET.json()
...
def waterleak_run_leak_detection_identify_leaks(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/run_identify_leaks')
		run_ld2 = reqGET.json()
...
def waterleak_get_user_leak_points(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/get_user_leak_points')
...
def waterleak_save_user_leak_points(request, roi_id, simulation_id, leak_detection_id):
	try:
		...
		reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/save_user_leak_points', json=payload)
		cs = reqPOST.json()
		print(cs)
		if cs['alert'] == 'created':
			s = cs['user_leak']
...
def waterleak_show_leak_points(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/show_leak_points', params=request.GET)
		get_lps = reqGET.json()
...
def waterleak_show_user_leak_point(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/show_user_leak_point', params=request.GET)
		get_lps = reqGET.json()
		return HttpResponse(json.dumps(get_lps), content_type='application/json')
...
def waterleak_delete_user_leak_points(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/delete_user_leak_points', params=request.GET)
		get_lps = reqGET.json()
		return HttpResponse(json.dumps(get_lps), content_type='application/json')
...
def waterleak_delete_a_user_leak_point(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/delete_a_user_leak_point', params=request.GET)
		get_lps = reqGET.json()
		return HttpResponse(json.dumps(get_lps), content_type='application/json')
...
def waterleak_edit_user_leak_point(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/edit_user_leak_point', params=request.GET)
		get_lps = reqGET.json()
		return HttpResponse(json.dumps(get_lps), content_type='application/json')
...
def waterleak_download_user_leak_point(request, roi_id, simulation_id, leak_detection_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/download_user_leak_point', params=request.GET)
		show_request = reqGET.json()
		response = HttpResponse(show_request['kml'], content_type='application/vnd.google-earth.kml+xml')
		response['Content-Disposition'] = 'attachment; filename="{0}"'.format(simulation.aos.name+'-'+show_request['filename'])
		return response
...
def waterleak_clone_user_leak_points(request, roi_id, simulation_id, leak_detection_id):
	try:
		...
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		leakdetection = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation=simulation)
		reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/leak_detections/'+str(leakdetection.interm_leak_detection_id)+'/clone_user_leak_points', json=jsonReq)
...
def run_simulation(request, roi_id, simulation_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/run')
...
def restart_simulation(request, roi_id, simulation_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/restart')
...
def update_simulation(request, roi_id, simulation_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/update')
...
def stop_simulation(request, roi_id, simulation_id):
	try:
		simulation = worsica_portal_models.Simulation.objects.get(id=simulation_id, aos__id=roi_id)
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/stop')
...
def delete_simulation(request, roi_id, simulation_id):
	try:
		...
		if not is_owned_by_user:
			...
			reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/delete')
			delete_simulation = reqGET.json()
			if(delete_simulation['state'] == 'deleted'):
				#simulation.is_visible=False
				#simulation.save()
				simulation.delete()
...
def waterleak_get_compatible_pipe_networks(request, roi_id):
	...
	_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(request.user.id)+'/list?service_type=waterleak'
		...
		if simulation.job_submission_id is not None:
			...
			if show_simulation['alert']!='error':
				if show_simulation['state']=='success':
					if show_simulation['leastrecentdate']!=None and show_simulation['mostrecentdate']!=None:
						jsimulations.append({
							'simulation_id': simulation.id,
							'name': simulation.name,
							'beginDate': show_simulation['leastrecentdate'],#not so sure...
							'endDate': show_simulation['mostrecentdate'],#not so sure...
						...
						jsimulations.append({
							'simulation_id': simulation.id,
							'name': simulation.name,
							'beginDate': blob['step2Inputs']['beginDate'],#not so sure...
							'endDate': blob['step2Inputs']['endDate'],#not so sure...
							'job_submission_id': simulation.job_submission_id,
						})
...
def waterleak_get_leak_detections(request, roi_id, simulation_id):
	...
	for ld in worsica_portal_models.LeakDetection.objects.filter(simulation_id = simulation_id, is_visible=True):
		if ld.interm_leak_detection_id is not None:
			...
			simulation = worsica_portal_models.Simulation.objects.get(id=int(blob['step1ImageSelection']['simulation_id']))
			jld.append({
				'name': ld.name, 
				'id': ld.id,
...
#@login_required
#def waterleak_get_leak_detection(request, roi_id, simulation_id, leak_detection_id):
#	try:
#		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation_id = simulation_id, is_visible=True)
#		if ld.interm_leak_detection_id is not None:		
#			return HttpResponse(json.dumps({'state': 'loaded', 'blob': json.loads(ld.blob_text)}), content_type='application/json')
#	except Exception as e:
#		return HttpResponse(json.dumps({'state': 'error','roi_id': roi_id, 'id': simulation_id, 'leak_detection_id':leak_detection_id, 'error':str(e)}), content_type='application/json')
...
def waterleak_get_leak_detection_details(request, roi_id, simulation_id, leak_detection_id):
	try:
		...
		ld = worsica_portal_models.LeakDetection.objects.get(id=leak_detection_id, simulation = simulation, is_visible=True)
		if ld.interm_leak_detection_id is not None:	
...
def waterleak_get_list_imagesets(request, roi_id, simulation_id):
	try:
		...
		if simulation.job_submission_id is not None:
			...
			reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/job_submissions/'+str(simulation.job_submission_id)+'/list_imagesets', params=request.GET)
			j = reqGET.json()
			jimagesets = []
			if (j['alert'] != 'error'):
				#list imagesets
				jimagesets = [{"type": 'existing', "processState": js['processState'], "name": js['name'], "small_name": js['small_name'], "id": js['id'], "date":js['sensingDate'], 'uuids':js['uuids'] } for js in j['processedImagesets']]
				#list esa images
				if j['leastrecentdate']!=None and j['mostrecentdate']!=None:
					dtjleastrecentdate = datetime.datetime.strptime(j['leastrecentdate'], '%Y-%m-%d')-datetime.timedelta(days=1)
					dtjmostrecentdate = datetime.datetime.strptime(j['mostrecentdate'], '%Y-%m-%d')+datetime.timedelta(days=1)
					...
					#print(dtleastrecentdate.strftime('%Y%m%d'))
					#print(dtmostrecentdate.strftime('%Y%m%d'))
					#print(dtbegin_date_ld.strftime('%Y%m%d'))
					#print(dtend_date_ld.strftime('%Y%m%d'))
					uX,uY = simulation.aos.upperXcoordinate, simulation.aos.upperYcoordinate #float(request.GET.get('uX')), float(request.GET.get('uY'))
					lX,lY = simulation.aos.lowerXcoordinate, simulation.aos.lowerYcoordinate #float(request.GET.get('lX')), float(request.GET.get('lY'))
					minCloudCov, maxCloudCov = 0, 100
					platformname = 'Sentinel-2'#request.GET.get('platformname')
					level = "Level-1C,Level-2A" #(request.GET.get('processinglevel') if platformname == 'Sentinel-2' else request.GET.get('producttype')) #'Level-1C,Level-2A'
					#level = request.GET.get('producttype') #'GRD,'
					#sensorOM = request.GET.get('sensoroperationalmode') #'IW,'
					esaFail = False
					esaFail1 = False
					...
					if dtbegin_date_ld<dtleastrecentdate:
						print('dtbegin_date_ld='+str(dtbegin_date_ld))
						print('dtleastrecentdate='+str(dtleastrecentdate))
						
						#search before least recent
						print('===========ESA search before least recent=============')
						bD,eD = dtbegin_date_ld.strftime('%Y%m%d').replace("-",""), dtleastrecentdate.strftime('%Y%m%d').replace("-","")
						try:
							imageryJson = _start_esa_imagery_search(bD, eD, uX, uY, lX, lY, minCloudCov, maxCloudCov, platformname, level, None)
							sorted_imageryJson = sorted(imageryJson, key=itemgetter('date'))
							for key, group in itertools.groupby(sorted_imageryJson, key=lambda x:x['date']):#.split(' ')[0]):
								lgroup = list(group)
								#print(lgroup)
					...
					if dtend_date_ld>dtmostrecentdate:
						...
						bD,eD = dtmostrecentdate.strftime('%Y%m%d').replace("-",""), dtend_date_ld.strftime('%Y%m%d').replace("-","")
						try:
							imageryJson = _start_esa_imagery_search(bD, eD, uX, uY, lX, lY, minCloudCov, maxCloudCov, platformname, level, None)
							sorted_imageryJson = sorted(imageryJson, key=itemgetter('date'))
							for key, group in itertools.groupby(sorted_imageryJson, key=lambda x:x['date']):#.split(' ')[0]):
								lgroup = list(group)
								#print(lgroup)
								#key = 2021-02-13 11:21:51.024000
								key_ymd = key.split(' ')[0].replace("-","") #20210213
								#date_ymd_hms = key.split('.')[0] #2021-02-13 11:21:51
								jimagesets.append({
									"type": "new",
									"processState": None,
									"name": "esa_images_"+str(key_ymd),
...
def proxy_intermediate_show_user_repository(request, service_type):
	...
	try:	
		#print(user_id)	
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/show')
		...
		try:
			...
			if not myfilenametmp.endswith('.zip'):
				...
				if (hasSHP and hasDBF and hasSHX):
					...
					print('[proxy_intermediate_preview_user_repository_geometry]: upload file directly to worsica intermediate')
					f = open(myfilenametmp, 'rb')
					files = {'myfile': f }				
					#payload = {'user_id': user_id, 'filename': myfilename+'.zip', 'type': 'geometry'}
					reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/geometries/preview', files=files )				
		...
		try:
			...
			if not myfilenametmp.endswith('.zip'):
				...
				if (hasSHP and hasDBF and hasSHX):
					...
					print('[proxy_intermediate_upload_user_repository_geometry]: upload file directly to nextcloud')
					#files = {'myfile': open(myfilenametmp, 'rb')}				
					auth = (nextcloud_access.NEXTCLOUD_USER, nextcloud_access.NEXTCLOUD_PWD)
					with open(myfilenametmp, 'rb') as f:
						r = requests.put(nextcloud_access.NEXTCLOUD_URL_PATH+'/user_repository/user'+str(user_id)+'/geometries/'+service_type+'/'+myfilename+'.zip', data=f, auth=auth)
						if (r.status_code == 201 or r.status_code == 204):
							print('[proxy_intermediate_upload_user_repository_geometry]: upload successful')
							print('[proxy_intermediate_upload_user_repository_geometry]: start storing on intermediate side')
							payload = {'user_id': user_id, 'filename': myfilename+'.zip', 'type': 'geometry'}
							reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/geometries/upload', json=payload )				
							response = reqPOST.json()					
							print('[proxy_intermediate_upload_user_repository_geometry]: success')
							return HttpResponse(json.dumps(response), content_type='application/json')
						else:
							print('[proxy_intermediate_upload_user_repository_geometry]: upload error '+str(r.status_code))
...
def proxy_intermediate_show_user_repository_geometry(request, service_type, geometry_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/geometries/'+str(geometry_id))
		response = reqGET.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
...
def proxy_intermediate_delete_user_repository_geometry(request, service_type, geometry_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/geometries/'+str(geometry_id)+'/delete')
		response = reqGET.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'alert': 'error','geometry_id': geometry_id, 'error':str(e)}), content_type='application/json')
	...
	if request.method == 'POST':
		try:
			...
			if not myfilenametmp.endswith('.tif'):
				...
				zipfile.ZipFile(myfilenamezip, mode='w').write(myfilenametmp, arcname=newfilename+'.tif', compress_type=zipfile.ZIP_DEFLATED)
				...
				#files = {'myfile': open(myfilenamezip, 'rb')}
				auth = (nextcloud_access.NEXTCLOUD_USER, nextcloud_access.NEXTCLOUD_PWD)
				with open(myfilenametmp, 'rb') as f:
					r = requests.put(nextcloud_access.NEXTCLOUD_URL_PATH+'/user_repository/user'+str(user_id)+'/masks/'+service_type+'/'+newfilename+'.zip', data=f, auth=auth)
					if (r.status_code == 201 or r.status_code == 204):
						print('[proxy_intermediate_upload_user_repository_mask]: upload successful')
						print('[proxy_intermediate_upload_user_repository_mask]: start storing on intermediate side')
						payload = {'user_id': user_id, 'filename': newfilename+'.zip', 'type': 'mask'}
						reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/masks/upload', json=payload )				
						response = reqPOST.json()					
						print('[proxy_intermediate_upload_user_repository_mask]: success')
						return HttpResponse(json.dumps(response), content_type='application/json')
					else:
						print('[proxy_intermediate_upload_user_repository_mask]: upload error '+str(r.status_code))
						return HttpResponse(json.dumps({'alert': 'error', 'message': 'error '+str(r.status_code), 'details': 'error '+str(r.status_code)}), content_type='application/json')
...
def proxy_intermediate_show_user_repository_mask(request, service_type, mask_id, raster_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/masks/'+str(mask_id)+'/'+str(raster_id))
		response = reqGET.json()
		response['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+response['response']['url'])
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
...
def proxy_intermediate_delete_user_repository_mask(request, service_type, mask_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/masks/'+str(mask_id)+'/delete')
		response = reqGET.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
	...
	if request.method == 'POST':
		try:
			...
			if not myfilenametmp.endswith('.kml'):
				...
				myfilenamezip = '/tmp/'+newfilename+'.zip'
				zipfile.ZipFile(myfilenamezip, mode='w').write(myfilenametmp, arcname=newfilename+'.kml', compress_type=zipfile.ZIP_DEFLATED)
				...
				auth = (nextcloud_access.NEXTCLOUD_USER, nextcloud_access.NEXTCLOUD_PWD)
				with open(myfilenametmp, 'rb') as f:
					r = requests.put(nextcloud_access.NEXTCLOUD_URL_PATH+'/user_repository/user'+str(user_id)+'/leakpoints/'+service_type+'/'+newfilename+'.zip', data=f, auth=auth)
					if (r.status_code == 201 or r.status_code == 204):
						print('[proxy_intermediate_upload_user_repository_leak_point]: upload successful')
						print('[proxy_intermediate_upload_user_repository_leak_point]: start storing on intermediate side')
						payload = {'user_id': user_id, 'filename': newfilename+'.zip', 'type': 'leakpoint'}
						reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/leaks/upload', json=payload )				
						response = reqPOST.json()					
						print('[proxy_intermediate_upload_user_repository_leak_point]: success')
						return HttpResponse(json.dumps(response), content_type='application/json')
					else:
						print('[proxy_intermediate_upload_user_repository_leak_point]: upload error '+str(r.status_code))
						return HttpResponse(json.dumps({'alert': 'error', 'message': 'error '+str(r.status_code), 'details': 'error '+str(r.status_code)}), content_type='application/json')		
				os.remove(myfilenametmp)#delete tmp file
				os.remove(myfilenamezip)#delete zip file
...
def proxy_intermediate_show_user_repository_leak(request, service_type, leak_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/leaks/'+str(leak_id))
		response = reqGET.json()
		#response['response']['url'] = request.build_absolute_uri('/portal/proxy/intermediate'+response['response']['url'])
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
...
def proxy_intermediate_delete_user_repository_leak(request, service_type, leak_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/leaks/'+str(leak_id)+'/delete')
		response = reqGET.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'alert': 'error','leak_id': leak_id, 'error':str(e)}), content_type='application/json')
...
def proxy_intermediate_list_user_repository_imageset(request, service_type):
	...
	try:
		...
		roi_polygon = str(aos.upperYcoordinate)+'|'+str(aos.upperXcoordinate)+'|'+str(aos.lowerYcoordinate)+'|'+str(aos.lowerXcoordinate)
		list_url = 'http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/imagesets?roi_polygon='+roi_polygon
		reqGET = requests.get(list_url)
		j = reqGET.json()
		#if (j['alert'] != 'error'):
		#jsonResponseUserRepository.append(j)
	...
	if request.method == 'POST':
		try:
			...
			if not myfilenametmp.endswith('.zip'):
				...
				newfilename = 'uploaded-user'+str(user_id)+'_'+myfilename.replace('_','-') #USERID_FILENAME_YYYYmmddTHHMMSS
				#print('[proxy_intermediate_upload_user_repository_imageset]: generate zip file')
				#myfilenamezip = '/tmp/'+newfilename+'.zip'
				#zipfile.ZipFile(myfilenamezip, mode='w').write(myfilenametmp, arcname=newfilename+'.tif', compress_type=zipfile.ZIP_DEFLATED)
				...
				auth = (nextcloud_access.NEXTCLOUD_USER, nextcloud_access.NEXTCLOUD_PWD)
				with open(myfilenametmp, 'rb') as f:
					r = requests.put(nextcloud_access.NEXTCLOUD_URL_PATH+'/user_repository/user'+str(user_id)+'/imagesets/'+newfilename+'.zip', data=f, auth=auth)
					if (r.status_code == 201 or r.status_code == 204):
						print('[proxy_intermediate_upload_user_repository_imageset]: upload successful')
						print('[proxy_intermediate_upload_user_repository_imageset]: start storing on intermediate side')
						payload = {'user_id': user_id, 'filename': newfilename+'.zip', 'type': 'imageset'}
						reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/imagesets/upload', json=payload )				
						response = reqPOST.json()					
						print('[proxy_intermediate_upload_user_repository_imageset]: success')
						return HttpResponse(json.dumps(response), content_type='application/json')
					else:
						print('[proxy_intermediate_upload_user_repository_imageset]: upload error '+str(r.status_code))
						return HttpResponse(json.dumps({'alert': 'error', 'message': 'error '+str(r.status_code), 'details': 'error '+str(r.status_code)}), content_type='application/json')
				os.remove(myfilenametmp)#delete tmp file
				os.remove(myfilenamezip)#delete zip file
...
def proxy_intermediate_show_user_repository_imageset(request, service_type, imageset_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/imagesets/'+str(imageset_id))
		response = reqGET.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
...
def proxy_intermediate_show_thumbnail_user_repository_imageset(request, service_type, imageset_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/imagesets/'+str(imageset_id)+'/show_thumbnail')
		response = reqGET.json()
		print(response)
		#if response['alert']=='success':
		#	encodedImage = response['encodedImage']
		#	return HttpResponse(mark_safe("data: image/png; base64, {}".format(encodedImage)))
		#	#return HttpResponse(mark_safe('<img src="data: image/png; base64, {}">'.format(encodedImage)))
		#else:
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'alert': 'error','imageset_id': imageset_id, 'error':str(e)}), content_type='application/json')
...
def proxy_intermediate_edit_user_repository_imageset(request, service_type, imageset_id):
	...
	try:
		jsonReq = json.loads(request.body.decode('utf-8'))
		reqPOST = requests.post('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/imagesets/'+str(imageset_id)+'/edit', json=jsonReq)
		response = reqPOST.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'alert': 'error','imageset_id': imageset_id, 'error':str(e)}), content_type='application/json')
...
def proxy_intermediate_delete_user_repository_imageset(request, service_type, imageset_id):
	user_id = request.user.id
	try:
		reqGET = requests.get('http://'+settings.WORSICA_INTERMEDIATE_URL+'/api/user_repository/user'+str(user_id)+'/'+service_type+'/imagesets/'+str(imageset_id)+'/delete')
		response = reqGET.json()
		return HttpResponse(json.dumps(response), content_type='application/json')
	except Exception as e:
		return HttpResponse(json.dumps({'alert': 'error','imageset_id': imageset_id, 'error':str(e)}), content_type='application/json')
